...
class Chapter3OpeningTitle(OpeningTitle):
    CONFIG = {
        ...
        "chapter_str" : "Kapitola 3\\\\ Porovnávání množin, kontinuum",
    }
    ...
    def construct(self):
        ...
        for color, alpha, data in zip(color_gradient([GREEN, ORANGE], 4),
            ...
            for mobj in triangle:
                self.play(FadeIn(mobj), run_time = 0.375)
        ...
        set_title = TexMobject("|\\hbox{Podmnožiny }\\omega|")
        seq_title = TexMobject("=|\\hbox{Binární posloupnosti}|")
...
class FinitePowerSetScene(Scene):
    def construct(self):
        ...
        desc_P = TextMobject("$\\mathcal P$otenční\\\\ množina")
        ...
        for i in range(3):
            ...
            if i == 0: self.wait_to(35.5)
            elif i == 1: self.wait_to(40.6)
            elif i == 2: self.wait_to(44)
            self.play(ShowCreation(arrow_possib))

            if i == 0: self.wait_to(38)
        ...
        brace_desc = TextMobject("Vše z Cantorova\\\\ diagonálního\\\\ argumentu")
        brace.put_at_tip(brace_desc)

        for i, ineq in enumerate(inequalities):
            if i == 0: self.wait_to(60 + 13.5)
            elif i == 1: self.wait_to(60 + 16.5)
            elif i == 2: self.wait_to(60 + 18)
            elif i == 3: self.wait_to(60 + 19.8)
            elif i == 5: self.wait_to(60 + 25.5)
            elif i == 6: self.wait_to(60 + 32.7)
...
class CantorRevisited(Scene):
    def construct(self):
        ...
        self.wait_to(38.5)
        try_connect_M(0, 47.5)
        try_connect_M(2, 58.5)
...
class CantorBernsteinScene(Scene):
    ...
    def construct(self):
        ...
        descs_group = self.gradually_connect(seq, descs=("5", "\{5\}", "0.000001"),
                                             waits = (31.7, 39.9, 42, 46.3, 50.8, 56, 58))
        ...
        self.gradually_connect(one_sided_down[0], waits = (60 + 30,))
        self.play(self.to_side(seq))
        ...
        self.wait_to(3*60 + 1.5)
        title = TextMobject("Cantor-Bernsteinova věta")
        title.scale(1.3)
        title.to_edge(UP)
...
class RemainingQuestions(Scene):
    def construct(self):
        comparable = TextMobject("Jsou libovolné dvě množiny porovnatelné?")
        continuum = TextMobject("Je kontinuum nejmenší nespočetná mohutnost?")
        ...
        continuum_ans = TextMobject("Otevřené navždy...")
