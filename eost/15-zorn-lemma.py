...
class ZornLemmaExample1(Scene):
    def construct(self):
        ...
        wrong_lines_dest[1].shift(UP*(y_shifts[1] - y_shifts[0]))
...
class WellOrderingPrinciple(Scene):
    ...
    def construct(self):
        ...
        ori_line = reals.main_line
        reals.remove(ori_line)
        self.remove(ori_line)
        real_parts = reals.family_members_with_points()
        line_parts = GradientLine(
            ori_line.get_start(), ori_line.get_end(),
            BLUE, BLUE,
            segment_num = len(omega1_parts) - len(real_parts),
        )
        real_parts += line_parts.family_members_with_points()
        random.shuffle(real_parts)

        real_parts = VGroup(real_parts)
        real_parts_ori = real_parts.copy()
        real_parts.save_state()
