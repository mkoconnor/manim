...
class Chapter4OpeningTitle(OpeningTitle):
    CONFIG = {
        ...
        "chapter_str" : "Kapitola 4\\\\ Transfinitní rekurze\\\\ a dobré uspořádání",
    }
...
class PowerSetsScene(Scene):
    ...
    def construct(self):
        ...
        powerset_desc = TextMobject("$\\mathcal P$otence")
        ...
        for i, wait_time in enumerate((40.5, 44.5, 47.5)):
...
class FirstLimitStep(Scene):
    def construct(self):
        ...
        self.wait_to(30.5)
        self.play(Write(TextMobject("Transfinitní",  "rekurze").to_edge(DOWN)))
        self.wait_to(39)
...
class OrdinaryRecursion(Scene):
    ...
    def construct(self):
        ...
        title = TextMobject("Transfinitní",  "rekurze").to_edge(DOWN)
        ...
        base_case_title = TextMobject("Základ: $\\omega$")
        ...
        rec_step_title = TextMobject("Rekurzivní", " krok", ": ", "Potence", arg_separator = '')
        ...
        self.wait_to(11)
        self.play(Write(rec_step_title), run_time = 2.9)
        #self.wait_to(13.9)
        self.play(
        ...
        title[0].highlight(WHITE)
        succ_step_title = TextMobject("Izolovaný", " krok", ": ", "Potence", arg_separator = '')
        succ_step_title.set_color(succ_color)
        lim_step_title = TextMobject("Limitní krok: Sjednocení")
...
class RealsProblems(Scene):
    ...
    def construct(self):
        ...
        base_case_q = TextMobject("Základ rekurze?")
        ...
        base_case_a = TextMobject("Potřebujeme nejmenší prvek")
        ...
        succ_step_a = TextMobject("Musí existovat následníci")
    ...
    def pointer_with_skipped(self, x):
        ...
        skipped_desc = TextMobject("přeskočili jsme")
...
def ini_segment_brace(brace_base, **kwargs):
    return named_brace(brace_base, ("Počáteční", "úsek"), **kwargs)

def term_segment_brace(brace_base, **kwargs):
    return named_brace(brace_base, ("Koncový", "úsek"), **kwargs)

dec_seq_color = ORANGE

class WellOrderingConditionsObj(VMobject):

    def __init__(self):
        VMobject.__init__(self)

        self.title = TextMobject("Dobře uspořádaná množina").to_edge(UP)
        self.wrong_succ = TextMobject("Všechny", "počáteční úseky mají následníka")
        self.succ = TextMobject("Všechny", "vlastní", "počáteční úseky mají následníka").to_corner(UP+LEFT)
        self.term_min = TextMobject("$\Leftrightarrow{}$První prvky neprázdných koncových úseků").to_corner(UP+LEFT)
        self.inf_dec = TextMobject("$\Leftrightarrow{}$Žádná nekonečná klesající posloupnost").to_corner(UP+LEFT)
        self.set_min = TextMobject("$\Leftrightarrow{}$První prvky všech neprázdných podmnožin").to_corner(UP+LEFT)
...
class OmegaPlusZScene(Scene):
    ...
    def construct(self):
        ...
        self.wait_to(17.1)
        self.play(ShowCreation(omega_colorful[1]), run_time = 0.7)
        self.play(ShowCreation(omega_colorful[2]), run_time = 0.7)
        ...
        lim_step_a = TextMobject("Všechny počáteční úseky mají následníka")
        ...
        conditions = WellOrderingConditionsObj()
        self.wait_to(60 + 10.7)
        self.play(
            FadeOut(base_case_a),
            FadeOut(succ_step_a),
...
class WellOrderingCondition(Scene):
    ...
    def construct(self):
        ...
        any_set.shift(DOWN)
        any_set_desc = TextMobject("Jakákoli množina")
        ...
        self.fill_ordinal(OrdinalFiniteProd(OrdinalOmega, 4), wait_time = 22.5, one_range = 1)
        self.fill_ordinal(OrdinalFiniteProd(OrdinalOmega, 2), wait_time = 28.5, one_range = 1)
...
class OmegaSquaredScene(Scene):
    ...
    def add_brace(self, brace_type, show_creation = True): # 1
    ...
    def update_split(self, subset = False, play_run_time = 1):
        ...
        self.update_successor(run_time = play_run_time)
...
class ConditionsRecap(OmegaSquaredScene):
    ...
    def construct(self):
        ...
        self.cur_split = self.random_split()
        self.play(
            conditions.term_min.set_fill, None, 0.3,
            conditions.inf_dec.set_fill, None, 0.3,
            *self.highlight_split(animation = True)
        ) # 9.5
        ini_brace = self.add_brace(('ini', DOWN)) # 10.5

        self.add_successor(add_pointer = UP) # 11.5

        for _ in range(2):
            self.cur_split = self.random_split()
            self.update_split(play_run_time = 0.5)
            self.dither()
        ...
        self.play(*[
            conditions.term_min.set_fill, None, 1,
            ReplacementTransform(ini_brace, term_brace),
        ]+highlight_animation+[Animation(self.successor)]) # 17
        self.dither() # 18
        ...
        self.play(
            ...
            conditions.inf_dec.set_fill, None, 1,
