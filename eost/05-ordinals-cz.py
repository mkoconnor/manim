...
class Chapter5OpeningTitle(OpeningTitle):
    CONFIG = {
        ...
        "chapter_str" : "Kapitola 5\\\\ Ordinální čísla",
    }
class Chapter5OpeningQuote(OpeningQuote):
    CONFIG = {
        "quote" : [
            "Ono je to vlastně docela","\\relax triviální,","co není triviální, je to","pochopit.",
        ],
...
class SimultaneousRecursion(Scene):
    def construct(self):
        ...
        title = TextMobject("Dobře uspořádané množiny")
        title.to_edge(UP)
        ...
        for bar1, bar2, run_time, wait_time in\
            zip(ordinal1_fg_al[:3], self.ordinal2_fg[:3], (1,1,0.5), (42,44.5,46)):
        ...
        self.wait_to(60 + 33.7)
        ordinal_numbers_title = TextMobject("Ordinální čísla").to_edge(DOWN)
        self.play(Write(ordinal_numbers_title))
        self.wait_to(60 + 41.1)
...
class OrdinalMatching(Scene):
    def construct(self):
        ...
        title = TextMobject("Dobře uspořádané množiny").to_edge(UP)
        ordinal_numbers_title = TextMobject("Ordinální čísla").to_edge(DOWN)
        ...
        type_comparison = TexMobject("\\operatorname{typ}(A)",
                                     "=",
                                     "\\operatorname{typ}(B)")
        ...
        ordinals_title_dest.set_fill(opacity = 0.3)
        ...
        type_comparison_dest.set_fill(opacity = 0.3)

        title_dest = title.copy()
        title_dest.behind_edge(UP)
        ...
        cardinal_title = TextMobject("Kardinální čísla")
...
class FiniteOrInfinite(Scene):
    ...
    def construct(self):
        ...
        title_fin = TextMobject("Konečné dobře uspořádané množiny").to_edge(UP)
        ...
        desc = TexMobject("\\operatorname{typ}(X)","=","|X|", "= 5").to_edge(DOWN)
        ...
        title_inf = TextMobject("Nekonečné dobře uspořádané množiny").to_edge(UP)
        ...
        self.play(FadeIn(ordinal1), FadeIn(ordinal2)) # 24
        ...
        desc_ord = TexMobject("\\operatorname{typ}(A)","<","\\operatorname{typ}(B)")
        ...
        self.play(Write(desc_ord), run_time = 2)
        self.wait_to(36.8)

        desc_ord_ori = desc_ord.copy()
        self.play(desc_ord.highlight, DARK_GREY) # 27.8
        ...
        self.play(ShowCreation(icon_confirmed), run_time = 0.7)
...
class ApplicationDifference(Scene):
    def construct(self):
        ...
        title = TextMobject("Odlišné užití")
        title.to_edge(UP)
        line = Line(title.get_edge_center(DOWN) + 0.5*DOWN,
                    SPACE_HEIGHT*DOWN)

        self.play(FadeIn(line), FadeIn(title))
        card_title = TextMobject("Kardinály","měří","množiny")
        ord_title = TextMobject("Ordinály\\\\","reprezentují podstatu\\\\","dobře uspořádaných\\\\ množin")
...
class OrdinalByRecursion(Scene):
    ...
    def construct(self):
        ...
        zero_title = TextMobject("Základ rekurze:","0").to_corner(UP+LEFT)
        ...
        successor_title = TextMobject("Izolovaný krok:","+1").to_corner(UP+LEFT)
        ...
        limit_title = TextMobject("Limitní krok: supremum","všech předchozích")
        ...
        self.wait_to(60 + 26.7)
        ord_pic, brace = self.play_infinite_supremum(bar, waits = (60 + 31, 60 + 32))
        self.remove(bar)
    ...
    def play_infinite_supremum2(self, underlying_bar, brace, ord_num = 15, write = None):
        ...
        animations = [
            Transform(ordinals, ordinals_dest),
            ShowCreation(underlying_bar),
        ]
        if write is not None: animations.append(Write(write))
        self.play(*animations)
...
class OrdinalRoles(Scene):
    def construct(self):
        ...
        order_types_title = TextMobject("typy dobře uspořádaných množin")
        indices_title = TextMobject("indexy v","dobrém uspořádání")
        ...
        for con, role in zip(connections, roles):
            ...
            self.wait_to(28, assert_positive = False)
        ...
        self.move_brace(ord1[:4], TextMobject("čtyři"))
        self.wait_to(60 + 32)
        last_bar = ord1[3].copy().set_color(ORANGE)
        lb_pointer = TrianglePointer(color = ORANGE).next_to(last_bar, UP)
        lb_desc = TextMobject("čtvrtý").next_to(lb_pointer, UP).set_color(ORANGE)
        self.play(
        ...
        indices_title2 = TextMobject("indexy v","univerzálním","dobrém uspořádání")
        indices_title2.scale(0.85)
        indices_title2.shift(indices_title.get_edge_center(LEFT)
                             - indices_title2.get_edge_center(LEFT))
...
class SupremumRecap(OrdinalByRecursion):
    def construct(self):
        ...
        supremum, _ = self.play_infinite_supremum(waits = (3.7, 5.3))
        self.wait_to(16.5)
...
class SupremumReal(Scene):
    def construct(self):
        ...
        self.wait_to(39)
        self.play(*map(FadeOut, [arrow, supremum_desc]), run_time = 0.5)
